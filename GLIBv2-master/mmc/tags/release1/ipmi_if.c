...
// local data buffer
static LocalBuffer eventBuffer;
static LocalBuffer readBuffer;

int loop=0;
static ipmi_msg_t rqs;
static u08 rsp_data[25];
static u08 rsp_data_len;
...
void ipmi_init()
{
	...
	i2cSetLocalDeviceAddr(ipmb_address, TRUE);

	//slave operation
	i2cSetSlaveReceiveHandler( ipmb_get );
...
void ipmi_check_request()
{
   ...
   //first checksum
	if( checsum_verify( readBuffer.Buffer, 2, readBuffer.Buffer[2] ) != 0 )
    {
       return; //ignore request
	}

    if( readBuffer.Length == 0x07 )
    {
    }
	//second checksum
	if( (checsum_verify( readBuffer.Buffer, readBuffer.Length -1, readBuffer.Buffer[readBuffer.Length - 1] )) != 0 )
    {
       return; //ignore request
	}
...
void ipmi_response_send()
{
            ...
            switch( rqs.cmd )
            {
			    ...
			    case IPMI_GET_DEVICE_SDR_CMD:
                        ...
                        if( (rsp_data_len = ipmi_sdr_get( &rqs.data[2], rqs.data[4], rqs.data[5], rsp_data)) == 0xff)
                            error = IPMI_CC_PARAM_OUT_OF_RANGE;
						else endofsdr=1;  // activate SDR flag on last query
                        break;
                ...
                case IPMI_GET_SENSOR_READING_CMD:
					...
					//	else start_crate = 0x01; // waiting initilisation nat board
                        break;
                default:
						error  = IPMI_CC_INV_CMD;
                        break;
            ...
            switch( rqs.cmd )
            {
                ...
                case IPMI_READ_FRU_DATA_CMD:
                        if( (rsp_data_len = ipmi_fru_data_read( &rqs.data[1], rqs.data[3], rsp_data )) == 0xff )
                        		error = IPMI_CC_PARAM_OUT_OF_RANGE;
						else endoffru = 1;   // activate Read FRU flag on last query
            ...
            switch( rqs.cmd )
            {
             ...
             case IPMI_PICMG_CMD_SET_FRU_LED_STATE:
                        if( rqs.data[2] == BLUE_LED ){
                            //  if( rqs.data[3] == LED_OFF )   // modified by Frédéric Bompard (CPPM)
                              {
	...
	if( endofsdr == 1 && endoffru == 1) start_crate = 1;   // Both flags activated : build Atart_crate flag
...
u08 ipmi_get_device_id( )
{
  ...
  rsp_data[len++]  = 0x00 | MMC_FW_REL_MAJ; //Major Firmware revision
  rsp_data[len++]  = MMC_FW_REL_MIN; //Minor Firmware revision
  rsp_data[len++]  = MMC_IPMI_REL; //IPMI version 1.0
  rsp_data[len++]  = IPMI_MSG_ADD_DEV_SUPP; //Additional device support(commands and functions)
  rsp_data[len++]  = IPMI_CPPM_ID & 0xff; //Manufacturer ID
  rsp_data[len++]  = IPMI_CPPM_ID >> 8;
  rsp_data[len++]  = 0x00;
  rsp_data[len++]  = IPMI_CPPM_AMC_ID & 0xff;	//Product ID
  rsp_data[len++]  = IPMI_CPPM_AMC_ID >> 8;
...
void ipmi_event_send( u08 sensor_number, u08 evData )
{
	...
	//send netfn
	eventBuffer.Buffer[1] = IPMI_SENSOR_EVENT_NETFN << 2;
...
{
	u08 geog_addr = 0;
	u08 GAddr1 = 0,GAddr2 = 0,GAddr3 = 0;
	...
	/* configure GA pull-up as low output */
    sbi( GAPU_DDR, GAPU_PORT_PIN );
		...
		case GROUNDED:
			{
				switch( GAddr2 )
				{
					case GROUNDED:
						{
							if( GAddr3 == UNCONNECTED ) { geog_addr = 0x72;	break; }
							else  
							     break; 
						}
					case UNCONNECTED:
						{
							if( GAddr3 == GROUNDED ) 	{ geog_addr = 0x74;	break; }
							if( GAddr3 == UNCONNECTED ) { geog_addr = 0x76;	break; }
							else 
								break;
						}
					default:
		 ...
		 case UNCONNECTED:
			 {
				switch( GAddr2 )
				{
					case GROUNDED:
						{
							if( GAddr3 == GROUNDED )    { geog_addr = 0x78;	break; }
							if( GAddr3 == UNCONNECTED ) { geog_addr = 0x7A;	break; }
							else  
							     break; 
						}
					case UNCONNECTED:
						{
							if( GAddr3 == GROUNDED ) 	{ geog_addr = 0x7C;	break; }
							if( GAddr3 == POWERED ) 	{ geog_addr = 0x7E;	break; }
							else 
								break;
						}
					case POWERED:
						{
							if( GAddr3 == UNCONNECTED ) { geog_addr = 0x80;	break; }
							if( GAddr3 == POWERED ) 	{ geog_addr = 0x82;	break; }
							else 
								break;
						}
					default:
		...
		case POWERED:
			{
				switch( GAddr2 )
				{
					...
					case UNCONNECTED:
						{
							if( GAddr3 == UNCONNECTED ) { geog_addr = 0x84;	break; }
							if( GAddr3 == POWERED ) 	{ geog_addr = 0x86;	break; }
							else 
								break;
						}
					case POWERED:
						{
							if( GAddr3 == UNCONNECTED ) { geog_addr = 0x88;	break; }
							else 
								break;
						}
					default:
	...
	/* re-enable internal pull-up resistors */
	SFIOR &= ~0x04;
