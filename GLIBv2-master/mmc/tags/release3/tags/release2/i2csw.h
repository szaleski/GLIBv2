...
#define QDEL            { asm volatile("nop"); asm volatile("nop"); asm volatile("nop"); asm volatile("nop"); asm volatile("nop"); }      // i2c quarter-bit delay
#define HDEL            { QDEL; QDEL; }                                                                                                   // i2c half-bit delay


// Functions
void i2cswInit(void);                                            // initialize I2C interface pins
void i2cswSend(u08 device, u08 sub, u08 length, u08 *data);      // send I2C data to <device> register <sub>
void i2cswReceive(u08 device, u08 sub, u08 length, u08 *data);   // receive I2C data from <device> register <sub>
