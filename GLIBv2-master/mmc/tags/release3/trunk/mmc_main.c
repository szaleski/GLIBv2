...
//***********/
{
    ...
    if (0xff == EEPROM_read(0))   // MJ: Should we really load the EEPROM in the MMC code or should this be done once at production time?
        Write_FRU_Info_default(); // Write default value < FRU_information > in EEPROM

    i2cswInit();   //MJ remove this function from the user code
    leds_init();
    ...
    if (HotSwap == HOT_SWAP_CLOSED)
        sens[HOT_SWAP_SENSOR].state = 0x01;  //See MTCA.4, table 3-2, Response data, bit 0
    else
        sens[HOT_SWAP_SENSOR].state = 0x02;  //See MTCA.4, table 3-2, Response data, bit 1
    ...
    while(1)
    {
        ...
        case M2:                  // Activation request
        {
            ...
            if (flag_blink == 1)
            {
                ...
                if(first_send)
                {
                    ipmi_event_send(HOT_SWAP_SENSOR, HotSwap);    // send value hot_swap to the MCH board   //MJ: This message will be sent periodically. Is that intended?
                    first_send = 0;
                }
            }
